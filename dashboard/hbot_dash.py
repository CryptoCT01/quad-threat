...
# Absolute docker path — tmux hbotdash often has a bare PATH without /usr/local/bin,
# which makes every status poll fail ("docker: No such file") and paints false OFF.
def _docker_bin() -> str:
    env = os.environ.get("DOCKER_BIN") or ""
    candidates = [
        env,
    ...
    for c in candidates:
        if c and (c == "docker" or os.path.isfile(c)):
            return c
    return "docker"
...
def load_universe_file() -> Dict[str, Any]:
    ...
    default_commodities = [
        {"symbol": "XAU-USDT", "leverage": 20, "tier": 2},
        {"symbol": "CL-USDT", "leverage": 20, "tier": 2},
    ]
    default_stocks = [
...
def apply_universe() -> None:
    """Re-read universe.yml into the module globals. Scan used to freeze the
    pair list at process start, so new E1 names (ADA/BNB/AVAX) tagged PERSONAL."""
    global _UNIVERSE, CRYPTO_SYMBOLS, COMMODITY_SYMBOLS, STOCK_SYMBOLS
    global SYMBOLS, TIER_MAP, LEV_MAP, MAX_OPEN_POSITIONS, E1_SYMBOLS
    ...
    E1_SYMBOLS = [s for s in SYMBOLS if s not in E4_PAIRS and s not in E2_BASKET]
    for _p in E4_PAIRS:
        if _p not in SYMBOLS:
            SYMBOLS.append(_p)
        LEV_MAP[_p] = 100
        TIER_MAP.setdefault(_p, 1)
    for _p in sorted(E2_BASKET):
        if _p not in SYMBOLS:
            SYMBOLS.append(_p)
        LEV_MAP[_p] = 20
        TIER_MAP.setdefault(_p, 1)
        if _p in ("XAU-USDT", "CL-USDT"):
            if _p not in COMMODITY_SYMBOLS:
                COMMODITY_SYMBOLS.append(_p)
        elif _p not in CRYPTO_SYMBOLS:
            CRYPTO_SYMBOLS.append(_p)
    CRYPTO_SYMBOLS = list(dict.fromkeys(E4_PAIRS + list(CRYPTO_SYMBOLS)))
...
# Short TTL cache for engine liveness. NEVER poll via `hbot status --json`
# (docker exec -it stacks until the sock wedges and the UI paints OFF).
_status_cache: Tuple[float, Dict[str, Any]] = (0.0, {})
STATUS_TTL = 8.0
_e1_alive_sticky: Dict[str, Any] = {"alive": False, "ts": 0.0, "pid": None}
_e4_alive_sticky: Dict[str, Any] = {"alive": False, "ts": 0.0}
...
ARCHIVES_DIR = os.path.expanduser("~/Desktop/humming-bot/archives")
E2_CLOSES_CACHE_PATH = os.path.join(HBOT_DATA_DIR, "e2_closes_cache.json")


def _e4_sqlite_paths() -> List[str]:
    """Live E4 DB plus Session-46+ archives (fresh live DB can have TradeFill=0)."""
    import glob
    out: List[str] = []
    live = os.path.join(HBOT_E4_DATA_DIR, "conf_e4_pmm.sqlite")
    if os.path.isfile(live):
        out.append(live)
    for d in sorted(glob.glob(os.path.join(ARCHIVES_DIR, "e4-sqlite-*"))):
        p = os.path.join(d, "conf_e4_pmm.sqlite")
        if os.path.isfile(p) and p not in out:
            out.append(p)
    return out


def _container_alive_docker_top(container: str, needle: str, timeout: float = 8.0):
    """True/False from `docker top` (often snappier than exec when sock is loaded). None on fail."""
    try:
        r = subprocess.run(
            [_docker_bin(), "top", container],
...
def _e4_orders_from_sqlite() -> List[Dict[str, Any]]:
    """Open-ish LIMIT quotes from live E4 Order table (no docker)."""
    import sqlite3
    db = os.path.join(HBOT_E4_DATA_DIR, "conf_e4_pmm.sqlite")
    if not os.path.isfile(db):
        return []
    dead = ("CANCEL", "FILL", "FAIL", "EXPIRE", "COMPLETE", "REJECT")
    live_hint = ("CREATED", "OPEN", "NEW", "POST", "SUBMIT")
    out: List[Dict[str, Any]] = []
    try:
        con = sqlite3.connect(f"file:{db}?mode=ro", uri=True, timeout=2.0)
        con.row_factory = sqlite3.Row
        rows = con.execute(
            'SELECT symbol, order_type, amount, price, last_status, exchange_order_id, '
            'last_update_timestamp FROM "Order" ORDER BY last_update_timestamp DESC LIMIT 40'
        ).fetchall()
        con.close()
    except Exception:
        return []
    seen = set()
    for r in rows:
        st = str(r["last_status"] or "").upper()
        if any(x in st for x in dead):
            continue
        if not any(x in st for x in live_hint):
            continue
        sym = r["symbol"] or "?"
        if sym in seen:
            continue
        ...
        try:
            ...
            # Hummingbot often stores ×1e6
            if px > 1e7:
                px /= 1_000_000.0
            if amt > 1e3 and sym.startswith("BTC"):
                amt /= 1_000_000.0
            elif amt > 1e4:
                amt /= 1_000_000.0
        except Exception:
...
def _engine_dbs_expanded() -> list:
    """Live engine DBs plus newest archived E4 sqlite (Session 46 archive wiped live TradeFill)."""
    out = list(_ENGINE_DBS)
    seen = {os.path.realpath(s["path"]) for s in out if os.path.isfile(s["path"])}
    archives = sorted(
        glob.glob(os.path.expanduser("~/Desktop/humming-bot/archives/e4-sqlite-*/conf_e4_pmm.sqlite")),
        key=lambda p: os.path.getmtime(p),
        reverse=True,
    )
    for path in archives[:2]:  # newest 2 archives
...
def _docker_e4(cmd: str, timeout: float = 8.0) -> subprocess.CompletedProcess:
    return subprocess.run(
        [_docker_bin(), "exec", "hummingbot-e4", "bash", "-lc", cmd],
        capture_output=True, text=True, timeout=timeout,
    )


def _spawn_quickstart(container: str, v2_conf: str) -> None:
    """Headless V2 start that actually works (bypasses hbot/cli password wall)."""
    pw = str(HBOT_PASSWORD or "CHANGE_ME")
    subprocess.Popen(
        [
            _docker_bin(), "exec", "-d", "-w", "/home/hummingbot",
            "-e", "PYTHONPATH=/home/hummingbot",
            "-e", f"HBOT_PASSWORD={pw}",
            container,
...
def _e4_spawn_engine() -> None:
    """Start the E4 PMM via quickstart. E1 untouched."""
    _spawn_quickstart("hummingbot-e4", "conf_e4_pmm.yml")


def _e1_spawn_engine() -> None:
    """Start Engine 1 via quickstart (conf_v37_scalp_multi)."""
    _spawn_quickstart("hummingbot", "conf_v37_scalp_multi.yml")


def _container_pgrep(container: str, needle: str, timeout: float = 12.0):
    """(found, pid). found is None if docker hung — caller MUST keep sticky ON."""
    try:
        r = subprocess.run(
            [_docker_bin(), "exec", container, "pgrep", "-af", needle],
            capture_output=True, text=True, timeout=timeout,
        )
        pids = []
        for ln in (r.stdout or "").splitlines():
            if needle not in ln or "pgrep" in ln:
                continue
            parts = ln.strip().split(None, 1)
            if parts and parts[0].isdigit() and parts[0] != "1":
...
def _sticky_alive(store: Dict[str, Any], found, pid=None, hold_s: float = 600.0) -> tuple:
    """Update sticky. Returns (alive, uncertain). Timeout never flips OFF."""
    now = time.time()
    if found is None:
        alive = bool(store.get("alive")) and (now - float(store.get("ts") or 0) < hold_s)
        return alive, True
    store["alive"] = bool(found)
    store["ts"] = now
    if pid is not None:
        store["pid"] = pid
...
def _e4_alive_hostpgrep() -> bool:
    """Truth: pgrep conf_e4_pmm inside hummingbot-e4. Timeout keeps last ON."""
    found, _pid = _container_pgrep("hummingbot-e4", "conf_e4_pmm")
    alive, _unc = _sticky_alive(_e4_alive_sticky, found)
    return alive
...
def e4_bot_status(force: bool = False) -> Dict[str, Any]:
    ...
    try:
        found, _pid = _container_pgrep("hummingbot-e4", "conf_e4_pmm", timeout=8.0)
        live_src = "pgrep"
        if found is None:
            top = _container_alive_docker_top("hummingbot-e4", "conf_e4_pmm")
            if top is True:
                found, _pid = True, None
                live_src = "docker_top"
            elif top is False:
                found, _pid = False, None
                live_src = "docker_top"
        alive, uncertain = _sticky_alive(_e4_alive_sticky, found, hold_s=600.0)
        # After dash bounce, sticky may be cold while docker is slow — log heuristic.
        if (uncertain or not alive) and not alive:
            try:
                logp = os.path.expanduser(
                    "~/Desktop/humming-bot/hummingbot/e4/logs/logs_conf_e4_pmm.log"
                )
                if os.path.isfile(logp) and (time.time() - os.path.getmtime(logp) < 180):
                    with open(logp, "rb") as f:
                        f.seek(0, 2)
                        sz = f.tell()
                        f.seek(max(0, sz - 12000))
                        tail = f.read().decode("utf-8", "replace")
                    if "Created LIMIT" in tail:
                        alive = True
                        uncertain = True
                        live_src = "log"
                        _e4_alive_sticky["alive"] = True
                        _e4_alive_sticky["ts"] = time.time()
            except Exception:
                pass
        orders = _e4_orders_from_sqlite()
        pairs = []
        for o in orders:
            m = o.get("symbol") or o.get("market")
            if m and m not in pairs:
                pairs.append(m)
        quoting = bool(alive)
        out.update({
            "available": True,
            ...
            "running": quoting,
            "uncertain": uncertain,
            "liveness": ("timeout" if uncertain and live_src == "pgrep" else live_src),
            "orders": orders,
        ...
        out["error"] = str(e)[:180]
        if _e4_alive_sticky.get("alive"):
            out.update({"available": True, "engine": True, "quoting": True, "running": True, "uncertain": True})
        try:
            orders = _e4_orders_from_sqlite()
            pairs = []
            for o in orders:
...
def e4_bot_start(replace: bool = False) -> Dict[str, Any]:
    ...
    try:
        if replace or pre.get("engine"):
            try:
                _docker_e4("pkill -f conf_e4_pmm || true", timeout=10.0)
                time.sleep(1.0)
            except Exception:
                pass
        _e4_spawn_engine()
...
def e4_bot_stop() -> Dict[str, Any]:
    ...
    Kill the real process (`pkill -f conf_e4_pmm`) — covers both cli.engine and
    hummingbot_quickstart.py --headless --v2 conf_e4_pmm.yml. Never pid 1 / docker-init.
    Confirm with `_e4_alive_hostpgrep`.
    """
    with _e4_op_lock:
    ...
    try:
        ...
        try:
            # Real E4 cmdline is quickstart --v2 conf_e4_pmm.yml (or older cli.engine).
            # Never docker restart; never kill docker-init / pid 1.
            _docker_e4(
                "pids=$(pgrep -f conf_e4_pmm || true); "
                "for p in $pids; do [ \"$p\" != 1 ] && kill \"$p\" 2>/dev/null || true; done; "
                "pkill -f conf_e4_pmm || true",
                timeout=10.0,
            )
        except Exception:
            pass
...
def cli_bot_status(force: bool = False) -> Dict[str, Any]:
    """Liveness via 2s docker pgrep. Never `hbot status --json` on the poll path."""
    ...
    found, pid = _container_pgrep("hummingbot", "conf_v37_scalp_multi", timeout=8.0)
    if found is None:
        top = _container_alive_docker_top("hummingbot", "conf_v37_scalp_multi")
        if top is True:
            found, pid = True, _e1_alive_sticky.get("pid")
        elif top is False:
            found, pid = False, None
    running, uncertain = _sticky_alive(_e1_alive_sticky, found, pid=pid, hold_s=600.0)
    with _bot_op_lock:
        phase = _bot_op.get("phase") or "idle"
    if running:
        status = "RUNNING"
        note = "timeout-sticky" if uncertain else "running"
        if phase == "stopping":
            status, note = "STOPPING", "stopping"
    else:
        status = "STOPPED"
        note = "pgrep-empty"
        if phase == "starting":
            status, note = "STARTING", "starting"
        elif phase == "stopping":
            status, note = "STOPPING", "stopping"
    data = {
        "running": running,
        "pid": pid or _e1_alive_sticky.get("pid"),
        "config": "conf_v37_scalp_multi.yml",
        "type": "controller",
        "strategy": "v2_with_controllers",
        "name": "conf_v37_scalp_multi",
    }
    result = {
        "ok": True,
        "running": running,
        ...
        "op_phase": phase,
        "uncertain": uncertain,
        "liveness": "timeout" if uncertain else "pgrep",
        "cli": data,
        "api": {},
        "raw": None,
    }
...
def cli_bot_start(config: str = BOT_CONFIG, replace: bool = False) -> Dict[str, Any]:
    """Start Engine 1 via hummingbot_quickstart --headless --v2 (not hbot CLI)."""
    ...
    if replace or pre.get("running"):
        try:
            subprocess.run(
                [
                    _docker_bin(), "exec", "hummingbot", "bash", "-lc",
                    "pids=$(pgrep -f conf_v37_scalp_multi || true); "
                    "for p in $pids; do [ \"$p\" != 1 ] && kill \"$p\" 2>/dev/null || true; done; "
                    "pkill -f conf_v37_scalp_multi || true",
                ],
                capture_output=True,
                text=True,
                timeout=10,
    ...
    return {
        ...
        "config": config,
        "running": running,
        "message": "Bot RUNNING" if running else "Bot start issued — still initializing (poll status)",
        "spawn": "quickstart",
    }
...
def cli_bot_stop() -> Dict[str, Any]:
    """
    Stop Engine 1 WITHOUT force-closing exchange positions.

    Kill the real E1 process (`pkill -f conf_v37_scalp_multi`, and
    hummingbot.cli.engine if present). Covers both cli.engine and
    hummingbot_quickstart.py --headless --v2 conf_v37_scalp_multi.
    NEVER docker restart. NEVER kill docker-init / pid 1.
    Positions stay open on Bitget.
    """
    ...
    try:
        subprocess.run(
            [_docker_bin(), "exec", "hummingbot", "bash", "-lc",
             ...
             "for p in $pids; do [ \"$p\" != 1 ] && kill \"$p\" 2>/dev/null || true; done; "
             "pkill -f conf_v37_scalp_multi || true; "
             "pkill -f hummingbot.cli.engine || true"],
    ...
    # Invalidate journal/perf/status cache so next poll does not show stale RUNNING
...
def _bitget_ssl_context():
    """macOS Frameworks Python often lacks system CAs; certifi fixes public Bitget GETs."""
    try:
        import certifi
        import ssl
...
def bitget_get(path: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=8, context=_bitget_ssl_context()) as resp:
...
def overlay_bot_truth(scan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not parsed.get("available"):
        ...
        return scan
    scan["bot_truth"] = True
    raw_open = [normalize_symbol(p) for p in (parsed.get("open_pairs") or []) if p]
    live_book = set()
    try:
        ...
        for p in (pos_resp.get("data") or []) if isinstance(pos_resp, dict) else []:
            try:
                ...
                amt = 0.0
            tp = p.get("trading_pair") or p.get("symbol")
            if tp and amt > 1e-12:
                live_book.add(normalize_symbol(str(tp)))
    except Exception:
        live_book = set()
    e2_live = {normalize_symbol(p) for p in (load_engine2_open_pairs() or [])}
    e4_live = {"BTC-USDT", "ETH-USDT"}
    if live_book:
        e1_open = [p for p in raw_open if p in live_book and p not in e2_live and p not in e4_live]
    else:
        e1_open = list(raw_open)
    scan["bot_signals_meta"] = {
        "enabled": parsed.get("enabled") or [],
        "score_threshold": parsed.get("score_threshold"),
        "open_pairs": e1_open,
        "phantom_open_pairs": [p for p in raw_open if p not in e1_open],
    }
    ...
    mx = int((parsed.get("slots") or {}).get("max") or MAX_OPEN_POSITIONS or 3)
    used = len(e1_open)
    scan["slots"] = {
        "used": used,
        "max": mx,
...
def build_scan(force: bool = False) -> Dict[str, Any]:
    ...
    cached_syms = ((cached or {}).get("universe") or {}).get("symbols") or []
    if not force and cached and now - ts < SCAN_TTL and cached_syms == SYMBOLS:
        return cached
    ...
    for pair in SYMBOLS:
        ...
        market.append({
            **t,
            ...
            "asset_class": (
                "commodity" if pair in COMMODITY_SYMBOLS or pair in ("XAU-USDT", "CL-USDT")
                else ("stock" if pair in STOCK_SYMBOLS else "crypto")
            ),
            "engine": "E4" if pair in E4_PAIRS else ("E2" if pair in E2_BASKET else "E1"),
        })
    ...
    for m in market:
        ...
        # Fallback READY never paints trade decisions. Radar bars still use by_strategy.
    ...
    out = {
        ...
        "universe": {
            "symbols": SYMBOLS,
            "e1_symbols": E1_SYMBOLS,
            "e2_symbols": sorted(E2_BASKET),
            "e4_symbols": E4_PAIRS,
...
def _load_e1_signals_disk() -> Optional[Dict[str, Any]]:
    """cli.engine does not populate format_status. Controller writes e1_signals.json each tick."""
    path = os.path.expanduser("~/Desktop/humming-bot/hummingbot/data/e1_signals.json")
    try:
        if not os.path.isfile(path):
            return None
        ...
        if not isinstance(d, dict) or not d.get("available"):
            return None
        d["source"] = d.get("source") or "e1_signals.json"
        d["age_s"] = round(age, 1)
        if not d.get("slots") and d.get("open_pairs") is not None:
            used = len(d.get("open_pairs") or [])
            mx = int(d.get("max_open_positions") or MAX_OPEN_POSITIONS or 3)
            d["slots"] = {"used": used, "max": mx, "free": max(0, mx - used), "over": used > mx}
        return d
    except Exception:
        return None


def _e1_engine_alive() -> bool:
    try:
        pr = subprocess.run(
            [_docker_bin(), "exec", "hummingbot", "bash", "-lc",
             "pgrep -f 'hummingbot.cli.engine --name conf_v37_scalp_multi' >/dev/null && echo ALIVE || true"],
            capture_output=True, text=True, timeout=5,
        )
        return "ALIVE" in (pr.stdout or "")
    except Exception:
        return False


def _strip_phantom_e1_opens(parsed: Dict[str, Any]) -> Dict[str, Any]:
    """Drop e1_signals open_pairs that are not live E1 legs on Bitget."""
    if not isinstance(parsed, dict) or not parsed.get("available"):
        return parsed
    raw = [normalize_symbol(p) for p in (parsed.get("open_pairs") or []) if p]
    live_book: set = set()
    try:
        pos_resp = hbot_request("POST", "/trading/positions", {
            "account_names": ["master_account"],
    ...
    e2_live = {normalize_symbol(p) for p in (load_engine2_open_pairs() or [])}
    e4_live = {"BTC-USDT", "ETH-USDT"}
    e1_open = [p for p in raw if p in live_book and p not in e2_live and p not in e4_live]
    out = dict(parsed)
    out["open_pairs"] = e1_open
    out["phantom_open_pairs"] = [p for p in raw if p not in e1_open]
    mx = int((out.get("slots") or {}).get("max") or MAX_OPEN_POSITIONS or 3)
    used = len(e1_open)
    out["slots"] = {"used": used, "max": mx, "free": max(0, mx - used), "over": used > mx}
    return out


def parse_bot_signals() -> Dict[str, Any]:
    return _strip_phantom_e1_opens(_parse_bot_signals_raw())


def _parse_bot_signals_raw() -> Dict[str, Any]:
    """Authoritative live signals. cli.engine has no format_status — read e1_signals.json."""
    disk = _load_e1_signals_disk()
    if disk and disk.get("source") not in (None, "probe"):
        return disk
    if _e1_engine_alive():
        return {
            "available": True,
            "source": "pgrep",
            "reason": "waiting_controller_dump",
            "candidates": [],
            "open_pairs": [],
            "enabled": ["SUPER_A", "ROC_RSI", "BB_VOL"],
            "score_threshold": 0.66,
            "slots": {"used": 0, "max": MAX_OPEN_POSITIONS, "free": MAX_OPEN_POSITIONS, "over": False},
        }
    try:
    ...
    # cli.engine never fills format_status — prefer the controller dump FIRST
    # so this endpoint cannot hang on `hbot status --json`.
    disk = _load_e1_signals_disk()
    if disk:
        return disk
...
def get_bot_signals() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"available": False, "reason": "parse_failed", "open_pairs": [], "candidates": []}
...
_E2_SESS_BASE = os.path.expanduser(
    "~/Desktop/humming-bot/condor/agents/v37_risk_manager/strategies/"
    "disciplined_perps_strategy/sessions"
)


def _latest_engine2_session_dir() -> str:
    import glob as _glob
    sessions = _glob.glob(os.path.join(_E2_SESS_BASE, "session_*"))
    if not sessions:
        return ""
    def _freshness(s: str) -> float:
        # Directory mtime is bumped on Condor boot for every session folder.
        # Journal / status.json / newest snapshot track the actually-running loop.
        t = 0.0
        for name in ("journal.md", "status.json"):
            p = os.path.join(s, name)
            if os.path.isfile(p):
                t = max(t, os.path.getmtime(p))
        snaps = _glob.glob(os.path.join(s, "snapshots", "*.md"))
        if snaps:
            t = max(t, max(os.path.getmtime(p) for p in snaps))
        return t
    return max(sessions, key=_freshness)
...
def _action_from_text(text: str) -> str:
    ...
    if not text:
        return "HOLD"
    m = _re.search(r"\b(HOLD|BUY|SELL|OPEN|CLOSE|LONG|SHORT|WAIT)\b", text, _re.I)
    return m.group(1).upper() if m else "HOLD"


def load_engine2_history(limit: int = 30) -> List[Dict[str, Any]]:
    """Latest Condor Engine 2 decisions from the current session journal.

    Snapshots no longer embed `{action, thesis}` JSON (they log tool calls +
    journal_write). History is sourced from journal.md Decisions, with a
    snapshot journal_write fallback.
    """
    import glob as _glob
    import re as _re
    import json as _json
    history: List[Dict[str, Any]] = []
    sess = _latest_engine2_session_dir()
    if not sess:
        return history
    session_name = os.path.basename(sess)
    journal_path = os.path.join(sess, "journal.md")
    if os.path.isfile(journal_path):
        try:
            text = open(journal_path, encoding="utf-8", errors="replace").read()
        except Exception:
            text = ""
        ...
        for part in parts:
            ...
            secs[title.strip()] = rest
        tick_time: Dict[int, str] = {}
        for m in _re.finditer(r"^- tick#(\d+) \| ([^|]+) \|", secs.get("Ticks", ""), _re.M):
            tick_time[int(m.group(1))] = m.group(2).strip()
        for m in _re.finditer(
            ...
            history.append({
                "tick": n,
                "time": tick_time.get(n) or m.group(2),
                "action": _action_from_text(body),
                "symbol": None,
                "side": None,
                "thesis": body,
                "file": session_name + "/journal.md",
                "session": session_name,
            })
        # Humming-e2 / some Condor builds only write ## Ticks (## Decisions empty).
        if not history:
            for m in _re.finditer(
                r"^- tick#(\d+) \| ([^|]+) \|[^|]*\|\s*(.*)$",
                secs.get("Ticks", ""),
        ...
        if history:
            history.sort(key=lambda x: int(x.get("tick") or 0), reverse=True)
            return history[:limit]
    # Fallback: parse snapshot journal_write JSON (entry_type=action).
    files = sorted(
        _glob.glob(os.path.join(sess, "snapshots", "*.md")),
    ...
    for filename in files:
        try:
            ...
            continue
        decision = None
        for block in _re.findall(r"```json\s*(\{.*?\})\s*```", text, _re.S):
            try:
            ...
            if not isinstance(obj, dict):
                continue
            if obj.get("entry_type") == "action" and (obj.get("text") or obj.get("reasoning")):
                decision = obj
                break
            if "action" in obj and "thesis" in obj:
                decision = obj
                break
        if not decision:
            continue
        text_body = decision.get("text") or decision.get("thesis") or ""
        tick_n = decision.get("tick")
        if tick_n is None:
            bm = _re.search(r"snapshot_(\d+)", os.path.basename(filename))
            tick_n = int(bm.group(1)) if bm else None
        history.append({
            "tick": tick_n,
            "file": os.path.basename(filename),
            "time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(os.path.getmtime(filename))),
            "action": decision.get("action") if "thesis" in decision else _action_from_text(text_body),
            "symbol": decision.get("symbol"),
            "side": decision.get("side"),
            "confidence": decision.get("confidence"),
            "thesis": text_body or decision.get("reasoning") or "",
...
def _read_risk_manager_status() -> Dict[str, Any]:
    ...
    try:
        hist = load_engine2_history(limit=1)
        if hist:
            latest = hist[0]
            d["last_action"] = {
                "action": latest.get("action") or "HOLD",
                "symbol": latest.get("symbol"),
                "side": latest.get("side"),
                "confidence": latest.get("confidence"),
                "thesis": (latest.get("thesis") or "")[:160],
...
def load_engine2_open_pairs() -> List[str]:
    """Pairs Engine 2 actually filled (in-container Bitget), from e2_fills.jsonl."""
    path = os.path.expanduser("~/Desktop/humming-bot/hummingbot/data/e2_fills.jsonl")
    pairs: List[str] = []
    seen = set()
    if not os.path.isfile(path):
        return pairs
    try:
        with open(path) as f:
            for line in f:
                ...
                if not obj.get("ok"):
                    continue
                pair = str(obj.get("pair") or obj.get("trading_pair") or "")
...
_E2_TPSL_LAST = 0.0
_E2_TPSL_CACHE: Dict[str, Any] = {}
_E2_TPSL_BUSY = False
_E2_CLOSE_LAST = 0.0
_E2_CLOSE_CACHE: List[Dict[str, Any]] = []
_E2_CLOSE_BUSY = False


def ensure_engine2_tpsl(force: bool = False) -> Dict[str, Any]:
    """Mechanical safety SL for live E2 fills. Never stack docker execs."""
    global _E2_TPSL_LAST, _E2_TPSL_CACHE, _E2_TPSL_BUSY
    now = time.time()
    if not force and (now - _E2_TPSL_LAST) < 90.0 and _E2_TPSL_CACHE:
        return dict(_E2_TPSL_CACHE)
    if _E2_TPSL_BUSY:
        return dict(_E2_TPSL_CACHE) if _E2_TPSL_CACHE else {"ok": True, "pending": True}
    _E2_TPSL_BUSY = True
    # Only run when E2 has a live fill still on the book.
    open_pairs = load_engine2_open_pairs()
    if not open_pairs and not force:
        _E2_TPSL_LAST = now
        _E2_TPSL_CACHE = {"ok": True, "skipped": True, "reason": "no_live_e2"}
        _E2_TPSL_BUSY = False
        return dict(_E2_TPSL_CACHE)
    script = "/home/hummingbot/scripts/_e2_live_place.py"
    env_pw = os.environ.get("HBOT_PASSWORD") or HBOT_PASSWORD
    try:
        proc = subprocess.run(
            [
                _docker_bin(), "exec", "-w", "/home/hummingbot",
                "-e", "PYTHONPATH=/home/hummingbot",
                "-e", f"HBOT_PASSWORD={env_pw}",
                "hummingbot", "python", script, "ensure_tpsl",
            ],
            capture_output=True,
            text=True,
            timeout=8,
        ...
        line = ""
        for candidate in reversed((proc.stdout or "").strip().splitlines() or [""]):
        ...
        if not line:
            out = {
                "ok": False,
                "error": (proc.stderr or proc.stdout or "no_output")[-400],
                "rc": proc.returncode,
            }
        else:
            out = json.loads(line)
    ...
    _E2_TPSL_LAST = now
    _E2_TPSL_CACHE = out if isinstance(out, dict) else {"ok": False, "raw": out}
    _E2_TPSL_BUSY = False
    return dict(_E2_TPSL_CACHE)


def _load_e2_closes_disk() -> List[Dict[str, Any]]:
    """Survive dash bounce when docker exec for E2 closes is wedged."""
    try:
        if not os.path.isfile(E2_CLOSES_CACHE_PATH):
            return []
        with open(E2_CLOSES_CACHE_PATH) as f:
            data = json.load(f) or {}
        trades = data.get("trades") or []
        return [t for t in trades if isinstance(t, dict)]
    except Exception as e:
        print(f"[e2] cache read: {e}")
        return []


def _refresh_engine2_closed_trades() -> None:
    """Docker-exec Bitget E2 closes. Never call on the HTTP thread without cache."""
    global _E2_CLOSE_LAST, _E2_CLOSE_CACHE, _E2_CLOSE_BUSY
    global _perf_last_fetch, _journal_last_fetch
    if _E2_CLOSE_BUSY:
        return
    _E2_CLOSE_BUSY = True
    script = "/home/hummingbot/scripts/_e2_live_place.py"
    env_pw = os.environ.get("HBOT_PASSWORD") or HBOT_PASSWORD
    since_ms = int(max(_perf_reset_ts(), 0) * 1000)
    payload = json.dumps({"since_ms": since_ms if since_ms > 0 else 0})
    out: List[Dict[str, Any]] = []
    try:
        proc = subprocess.run(
            [
                _docker_bin(), "exec", "-i", "-w", "/home/hummingbot",
                "-e", "PYTHONPATH=/home/hummingbot",
                "-e", f"HBOT_PASSWORD={env_pw}",
                "hummingbot", "python", script, "closes",
            ],
            input=payload,
            capture_output=True,
            text=True,
        ...
        for rec in (data.get("trades") or []):
            if not isinstance(rec, dict):
                continue
            pnl = float(rec.get("pnl") or 0)
            ts = float(rec.get("timestamp") or 0)
            result = "win" if pnl > 0 else ("loss" if pnl < 0 else "flat")
            fresh.append({
                "id": rec.get("id") or f"e2-bg-{rec.get('orderId')}",
                ...
                "leverage": int(rec.get("leverage") or 20),
                "pnl": round(pnl, 6),
                "pnl_usd": round(pnl, 6),
                "pnl_pct": 0.0,
                "fees": float(rec.get("fees") or 0),
                "filled_quote": float(rec.get("filled_quote") or 0),
                "close_type": rec.get("close_type") or "BITGET_CLOSE",
                "exit_reason": rec.get("close_type") or "BITGET_CLOSE",
                "result": result,
                "status": "CLOSED",
                "opened_at": "",
        ...
        # Never blank E2 on docker blip / empty payload — keep prior + disk cache.
        if fresh:
            out = fresh
            try:
                os.makedirs(os.path.dirname(E2_CLOSES_CACHE_PATH), exist_ok=True)
                with open(E2_CLOSES_CACHE_PATH, "w") as f:
                    json.dump({"ts": time.time(), "trades": out}, f)
            except Exception as e:
                print(f"[e2] cache write: {e}")
        else:
            out = list(_E2_CLOSE_CACHE) or _load_e2_closes_disk()
    except Exception:
        out = list(_E2_CLOSE_CACHE) or _load_e2_closes_disk()
    _E2_CLOSE_CACHE = out
    _E2_CLOSE_LAST = time.time()
    _E2_CLOSE_BUSY = False
    _perf_last_fetch = 0.0
    _journal_last_fetch = 0.0
...
def load_engine2_closed_trades() -> List[Dict[str, Any]]:
    """E2 closes from Bitget REST (not Condor snapshots — those almost never have pnl)."""
    global _E2_CLOSE_CACHE, _E2_CLOSE_LAST
    now = time.time()
    if not _E2_CLOSE_CACHE:
        disk = _load_e2_closes_disk()
        if disk:
            _E2_CLOSE_CACHE = disk
            _E2_CLOSE_LAST = now
    if now - _E2_CLOSE_LAST > 45 or not _E2_CLOSE_CACHE:
        if not _E2_CLOSE_BUSY:
            threading.Thread(target=_refresh_engine2_closed_trades, daemon=True).start()
    return list(_E2_CLOSE_CACHE)
...
def load_executor_trades(limit: int = 200) -> List[Dict[str, Any]]:
    ...
    for spec in _engine_dbs_expanded():
        db_path = spec["path"]
        engine = spec["engine"]
        ...
        if prefer == "fills" or (engine == "E1" and not e1_executor_closed):
            paths = [db_path]
            if engine == "E4":
                paths = _e4_sqlite_paths() or [db_path]
            for fpath in paths:
                fill_trades = load_trades_from_fills(
                    fpath, limit=max(limit, 200), engine=engine, strategy=strategy
                )
...
def compute_performance() -> Dict[str, Any]:
    ...
    # After a journal reset, empty sqlite is truth — do not paint stale API executors.
    if _perf_reset_ts() > 0:
        _perf_cache = {
            "win_rate": "—", "profit_factor": "—", "total_pnl": 0,
            "trades": 0, "sharpe": "—", "max_dd": "0%",
...
def _perf_reset_ms() -> float:
    try:
        with open(PERF_BASELINE_PATH) as f:
            t = float((json.load(f) or {}).get("reset_at") or 0)
        return t * 1000.0 if t and t < 1e12 else t
    except Exception:
        return 0.0


def _tradefill_volume(db_path: str, since_ms: float) -> Tuple[float, int]:
    import sqlite3
    ...
    try:
        con = sqlite3.connect(f"file:{db_path}?mode=ro", uri=True, timeout=3.0)
        n, s = con.execute(
            "SELECT COUNT(*), COALESCE(SUM(CAST(amount AS REAL)*CAST(price AS REAL)),0) "
            "FROM TradeFill WHERE timestamp>=?",
            (since_ms,),
        ).fetchone()
        con.close()
        return float(s or 0) / 1_000_000_000_000.0, int(n or 0)
    except Exception as e:
        print(f"[volume] {db_path}: {e}")
        return 0.0, 0


def traded_volume() -> Dict[str, Any]:
    """Filled notional since journal reset. E1+E4 TradeFill + E2 REST fills."""
    global _vol_cache, _vol_cache_ts
    now = time.time()
    if _vol_cache and now - _vol_cache_ts < 20:
        return _vol_cache
    since = _perf_reset_ms()
    e1, n1 = _tradefill_volume(os.path.join(HBOT_DATA_DIR, "conf_v37_scalp_multi.sqlite"), since)
    e4 = 0.0
    n4 = 0
    for e4p in (_e4_sqlite_paths() or [os.path.join(HBOT_E4_DATA_DIR, "conf_e4_pmm.sqlite")]):
        v, n = _tradefill_volume(e4p, since)
        e4 += v
        n4 += n
    for _arch in sorted(
        glob.glob(os.path.expanduser("~/Desktop/humming-bot/archives/e4-sqlite-*/conf_e4_pmm.sqlite")),
        ...
        a_vol, a_n = _tradefill_volume(_arch, since)
        e4 += a_vol
        n4 += a_n
    e2 = 0.0
    n2 = 0
    e2p = os.path.join(HBOT_DATA_DIR, "e2_fills.jsonl")
    try:
        if os.path.isfile(e2p):
            with open(e2p) as f:
                for line in f:
                    line = line.strip()
                    if not line:
                    ...
                    if since and ts and ts * 1000.0 < since:
                        continue
                    e2 += float(d.get("notional") or 0)
                    n2 += 1
    except Exception as e:
        print(f"[volume] e2 fills: {e}")
    total = e1 + e4 + e2
    _vol_cache = {
        "ok": True,
        "total": round(total, 2),
...
def _get_condor_jwt() -> str:
    ...
    try:
        proc = subprocess.run(
            [
                "bash", "-c",
                'cd ~/Desktop/humming-bot/condor && unset PYTHONPATH && export PATH=/Users/cryptot/.local/bin:/usr/bin:/bin && '
                '/Users/cryptot/.local/bin/uv run python -c \'from condor.web.auth import create_jwt; '
...
_bitget_acct_cache = {"ts": 0.0, "data": None}
_bitget_acct_busy = False



_rebate_cache = {"ts": 0.0, "data": {}}
_rebate_busy = False


def get_today_rebate() -> Dict[str, Any]:
    """50% of today's Bitget USDT-M bill fees (00:00 BST). Accrued, not credited."""
    global _rebate_busy
    now = time.time()
    cached = _rebate_cache.get("data") or {}
    if cached.get("rebate_accrued") is not None and now - float(_rebate_cache.get("ts") or 0) < 25:
        return cached
    if _rebate_busy:
        return cached
    _rebate_busy = True
    script = "/home/hummingbot/scripts/_bitget_rebate_today.py"
    try:
        for ctn in ("hummingbot-e4", "hummingbot"):
            try:
                proc = subprocess.run(
                    [
                        _docker_bin(), "exec", "-w", "/home/hummingbot",
                        "-e", "PYTHONPATH=/home/hummingbot",
                        "-e", f"HBOT_PASSWORD={HBOT_PASSWORD}",
                        ctn, "/opt/conda/envs/hummingbot/bin/python", script,
                    ],
            ...
            if data.get("ok") or data.get("rebate_accrued") is not None:
                _rebate_cache["ts"] = now
                _rebate_cache["data"] = data
                return data
    except Exception:
        pass
    finally:
        _rebate_busy = False
    return cached or {"ok": False, "fees_paid": 0.0, "rebate_accrued": 0.0, "rebate_pct": 0.5}


def get_bitget_futures_account() -> Dict[str, Any]:
    """Bitget mix `available` (what the app shows), not hummingbot crossedMaxAvailable."""
    global _bitget_acct_busy
    now = time.time()
    cached = _bitget_acct_cache.get("data") or {}
    if cached.get("available") is not None and now - float(_bitget_acct_cache.get("ts") or 0) < 20:
        return cached
    if _bitget_acct_busy:
        return cached
    _bitget_acct_busy = True
    try:
        data = {}
        for ctn in ("hummingbot-e4", "hummingbot"):
            try:
                proc = subprocess.run(
                    [
                        ...
                        "-e", "PYTHONPATH=/home/hummingbot",
                        "-e", f"HBOT_PASSWORD={HBOT_PASSWORD}",
                        ctn, "/opt/conda/envs/hummingbot/bin/python",
                        "/home/hummingbot/scripts/_bitget_acct.py",
                    ],
            ...
            if data.get("available") is not None:
                break
        if data.get("available") is not None:
            _bitget_acct_cache["ts"] = now
            _bitget_acct_cache["data"] = data
            return data
    except Exception:
        pass
    finally:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def handle_api_get(self):
        try:
            ...
            if path == "/api/account":
                acct = get_bitget_futures_account()
                self.send_json({"ok": True, **acct})
                return

            if path == "/api/rebate":
                self.send_json(get_today_rebate())
                return
            ...
            if path == "/api/engine2/status":
                ...
                import glob as _glob_e2
                _snap_base = os.path.expanduser(
                    "~/Desktop/humming-bot/condor/agents/v37_risk_manager/strategies/"
                    "disciplined_perps_strategy/sessions"
                )
                _snaps = _glob_e2.glob(os.path.join(_snap_base, "session_*", "snapshots", "*.md"))
                last_tick_ts = max((os.path.getmtime(p) for p in _snaps), default=None)
                if last_tick_ts is not None:
                    last_tick_ts = int(last_tick_ts)
                e2_open = load_engine2_open_pairs()
                # Never block this GET on docker exec (was painting E2 OFF).
                tpsl_ensure = dict(_E2_TPSL_CACHE) if _E2_TPSL_CACHE else {"ok": True, "pending": True}
                if e2_open:
                    threading.Thread(
                        target=ensure_engine2_tpsl, kwargs={"force": False}, daemon=True
                    ).start()
                self.send_json({
                    "ok": True,
                    ...
                    "frequency_sec": instance.get("frequency_sec") or 300,
                    "last_tick_ts": last_tick_ts,
                    "slots": {"used": len(e2_open) or instance.get("open_count", 0),
                    ...
                    "open_pairs": e2_open,
                    "tpsl_ensure": tpsl_ensure,
                })
                return

            if path == "/api/engine2/history":
                history = load_engine2_history(limit=50)
                sess = _latest_engine2_session_dir()
                self.send_json({
                    "ok": True,
                    "history": history,
                    "count": len(history),
                    "session": os.path.basename(sess) if sess else None,
                })
                return
    ...
    def handle_api_post(self):
        try:
            ...
            if path == "/api/e4/start":
                ...
                if not confirm:
                    self.send_json({
                        ...
                        "hint": "Engine 4 start places LIVE Bitget quotes on BTC+ETH only (12 bps LIMIT TP, 80 bps SL, 100x, ~$1 clip). Pass confirm_live=true after UI confirm.",
                    })
                    return
            ...
            if path == "/api/engine2/start":
                # Do not spawn a second session if one is already looping.
                try:
                    agents = condor_api_get("agents")
                    engine2 = next((a for a in (agents or []) if isinstance(a, dict) and a.get("slug") == "v37_risk_manager"), None)
                    strat = next((s for s in ((engine2 or {}).get("strategies") or [])
                                  if isinstance(s, dict) and s.get("slug") == "disciplined_perps_strategy"), {})
                    if strat.get("status") == "running":
                        inst = (strat.get("instances") or [{}])[0]
                        self.send_json({"ok": True, "already_running": True,
                                        "tick_count": inst.get("tick_count"),
                                        "model": inst.get("agent_key")})
                        return
                except Exception:
                    pass
                config = {
                    ...
                    "trading_context": (
                        "LIVE Bitget USDT-M perpetuals. Engine 2 is an independent LLM breakout "
                        "trader: every 300 seconds open only XAU/CL/DOGE/NEAR/LTC at 20x via "
                        "manage_executors. Exchange banks Engine 2 at 0.4% TP / 0.5% SL. "
                        "Do not CLOSE E1 on $0.80. Overlay flatten E1/E4 only on invalid thesis or 3% DD."
                    ),
                    "risk_limits": {
                        "max_position_size_quote": 30.0,
                        "max_open_executors": 3,
                        "max_drawdown_pct": 3.0,
                        "shutdown_drawdown_pct": 5.0,
                    },
                }
                result = condor_api_post(f"{_engine2_base}/start", {"chat_id": 0, "config": config})
                started = isinstance(result, dict) and (
                    bool(result.get("started") or result.get("ok") or result.get("already_running"))
                    and "error" not in result
                )
                self.send_json({"ok": started, "result": result})
                return
            if path == "/api/engine2/stop":
                result = condor_api_post(f"{_engine2_base}/stop", {})
                self.send_json({"ok": "error" not in result, "result": result})
                return
            if path in ("/api/engine2/run-now", "/api/engine2/tick"):
                # Immediate tick on the EXISTING session. Never POST /start
                # (that would spawn a second loop). Countdown is display-only.
                try:
                    agents = condor_api_get("agents")
                    engine2 = next((a for a in (agents or []) if isinstance(a, dict) and a.get("slug") == "v37_risk_manager"), None)
                    strat = next((s for s in ((engine2 or {}).get("strategies") or [])
                                  if isinstance(s, dict) and s.get("slug") == "disciplined_perps_strategy"), {})
                    if strat.get("status") != "running":
                        self.send_json({"ok": False, "error": "Engine 2 is not running — press ON first"}, 409)
                        return
                except Exception:
                    pass
                result = condor_api_post(f"{_engine2_base}/tick", {})
                if isinstance(result, dict) and result.get("error"):
                    self.send_json({"ok": False, "error": result.get("error"), "result": result}, 502)
                    return
                self.send_json({
                    "ok": True,
                    "ticked": True,
                    "already_ticking": bool(result.get("already_ticking")),
                    "queued": bool(result.get("queued")),
                    "session_num": result.get("session_num"),
